...
public class CourseController {
    ...
    public HttpResponse<List<CursoDTO>> getCoursesV3(final String casoDeUso) throws JsonParseException {
        ...
        if(casoDeUso.equals("4")) {
            throw new NullPointerException("This is my custom error");
        }
     ...
     */
    @Error
    public HttpResponse<JsonError> jsonError
        (HttpRequest request, JsonParseException jsonParseException) {
        JsonError error =
                new JsonError("Invalid JSON: " + jsonParseException.getMessage())
                .link(Link.SELF, Link.of(request.getUri()));
        return HttpResponse.<JsonError>status(HttpStatus.BAD_REQUEST,
                "Fix your json").body(error);
    }

    @Error(status = HttpStatus.INTERNAL_SERVER_ERROR)
    public HttpResponse<JsonError> notFound(HttpRequest request) {
        JsonError error = new JsonError("Person Not Found")
                .link(Link.SELF, Link.of(request.getUri()));
